...
class GameEndCheckerForNormal
{
    ...
    public static bool Prefix()
    {
        ...
        if (WinnerTeam != CustomWinner.Default)
        {
            ...
            if (WinnerTeam is not CustomWinner.Draw and not CustomWinner.None and not CustomWinner.Error)
            {
                ...
                {
                    var godArray = Main.EnumerateAlivePlayerControls().Where(x => x.Is(CustomRoles.God)).ToArray();

                    if (godArray.Length > 0)
                    {
                        ...
                        if (!isGodWinConverted)
                        {
                            ...
                            foreach (PlayerControl pc in godArray)
                            {
                                WinnerIds.Add(pc.PlayerId);
                            }
                        }
                ...
                {
                    foreach (var pc in Main.EnumeratePlayerControls().Where(x => x.IsPlayerCoven() || x.Is(CustomRoles.Enchanted) || Main.PlayerStates[x.PlayerId].IsNecromancer))
                    {
                        if (!WinnerIds.Contains(pc.PlayerId))
                            WinnerIds.Add(pc.PlayerId);
                    }
                ...
                foreach (var pc in Main.EnumeratePlayerControls().Where(x => x.Is(CustomRoles.Quota)).ToArray())
                {
    ...
    class NormalGameEndPredicate : GameEndPredicate
    {
        ...
        public static bool CheckGameEndByLivingPlayers(out GameOverReason reason)
        {
            ...
            else if (Main.AllAlivePlayerControls.Length > 0 && Main.EnumerateAlivePlayerControls().All(p => p.Is(CustomRoles.Lovers))) // If Lover is alive Lover wins
            {
                reason = GameOverReason.ImpostorsByKill;
                ResetAndSetWinner(CustomWinner.Lovers);
                return true;
...
class CandRGameEndPredicate : GameEndPredicate
{
    ...
    public static bool CheckGameEndByLivingPlayers(out GameOverReason reason)
    {
        ...
        if (!Main.EnumerateAlivePlayerControls().Any())
        {
            reason = GameOverReason.ImpostorsByKill;
            ResetAndSetWinner(CustomWinner.None);
